...
function processGameWords(startIndex = 0) {
    ...
    while (selected.length < 5 && currentIndex < lrcLines.length) {
        let foundValidInWindow = false;
        let targetLineIdx = -1;

        for (let offset = 0; offset <= 5; offset++) {
            let checkIdx = currentIndex + offset;
            if (checkIdx < lrcLines.length && lineValidity[checkIdx]) {
                if (checkIdx !== lastLineIdx && checkIdx !== lastLineIdx + 1 && checkIdx !== lastLineIdx - 1) {
                    targetLineIdx = checkIdx;
                    foundValidInWindow = true;
                    break;
                }
            }
        }

        if (foundValidInWindow && targetLineIdx !== -1) {
            ...
            if (validCandidatesInLine.length > 0) {
                ...
                lastLineIdx = targetLineIdx;
                currentIndex = targetLineIdx + 2; 
            } else {
                currentIndex++;
            }
        } else {
...
async function pauseForQuiz(targetObj) {
    ...
    options.forEach(opt => {
        ...
        btn.textContent = opt;
        btn.onclick = () => checkAnswer(opt === correctMeaning, btn, optionsContainer, correctMeaning);
...
function checkAnswer(isCorrect, selectedBtn, container, correctMeaning) {
...
function endGame() {
    ...
    document.getElementById('final-stats').innerHTML = `¡Has completado todas las preguntas de la canción!`;
}
